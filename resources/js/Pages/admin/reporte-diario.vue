...
const diferenciaColor = computed(() => {
    const d = props.cashBalance.diferencia;
    if (d < 0) return 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400';
    if (d > 0) return 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400';
...
<template>
    ...
    <AdminLayout>
        ...
        <div class="mx-auto max-w-5xl space-y-6 py-6">
            ...
            <div class="rounded-2xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-surface-dark">
                ...
                <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                    <div class="rounded-xl bg-gray-50 p-3 dark:bg-gray-800/50">
                        ...
                        <p class="text-lg font-bold text-gray-800 dark:text-white">{{ fmt(cashBalance.opening) }}</p>
                    </div>
                    <div class="rounded-xl bg-emerald-50 p-3 dark:bg-emerald-900/20">
                        <p class="text-xs text-emerald-600 dark:text-emerald-400">+ Ventas Efectivo</p>
                        <p class="text-lg font-bold text-emerald-700 dark:text-emerald-300">{{ fmt(cashBalance.cashSales) }}</p>
                    </div>
                    <div class="rounded-xl bg-sky-50 p-3 dark:bg-sky-900/20">
                        <p class="text-xs text-sky-600 dark:text-sky-400">+ Ingresos</p>
                        <p class="text-lg font-bold text-sky-700 dark:text-sky-300">{{ fmt(cashBalance.ingresos) }}</p>
                    </div>
                    <div class="rounded-xl bg-red-50 p-3 dark:bg-red-900/20">
                        <p class="text-xs text-red-600 dark:text-red-400">- Retiros</p>
                        <p class="text-lg font-bold text-red-700 dark:text-red-300">{{ fmt(cashBalance.withdrawals) }}</p>
                    </div>
                    <div class="rounded-xl bg-primary-50 p-3 dark:bg-primary-900/20">
                        <p class="text-xs text-primary-600 dark:text-primary-400">= Total Esperado</p>
                        <p class="text-lg font-bold text-primary-700 dark:text-primary-300">{{ fmt(cashBalance.expected) }}</p>
                    </div>
                </div>
                <!-- Descuadre -->
                <div class="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-2">
                    <div class="rounded-xl bg-gray-50 p-3 dark:bg-gray-800/50">
                        <p class="text-xs text-gray-400">Efectivo Real (Cierre)</p>
                        <p class="text-lg font-bold text-gray-800 dark:text-white">{{ fmt(cashBalance.efectivoCierre) }}</p>
                    </div>
                    <div class="rounded-xl p-3" :class="diferenciaColor">
                        <p class="text-xs font-semibold uppercase opacity-70">{{ diferenciaTitle }}</p>
            ...
            <div class="rounded-2xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-surface-dark">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div class="rounded-xl bg-blue-50 p-3 dark:bg-blue-900/20">
                        <p class="text-xs text-blue-600 dark:text-blue-400">Tarjeta</p>
                        <p class="text-lg font-bold text-blue-700 dark:text-blue-300">{{ fmt(digitalSales.card) }}</p>
                    </div>
                    <div class="rounded-xl bg-purple-50 p-3 dark:bg-purple-900/20">
                        <p class="text-xs text-purple-600 dark:text-purple-400">Transferencia</p>
                        <p class="text-lg font-bold text-purple-700 dark:text-purple-300">{{ fmt(digitalSales.transfer) }}</p>
                    </div>
                    <div class="rounded-xl bg-gray-50 p-3 dark:bg-gray-800/50">
                        <p class="text-xs text-gray-400">Total Digital</p>
                        <p class="text-lg font-bold text-gray-800 dark:text-white">{{ fmt(digitalSales.total) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-surface-dark">
                ...
                <div v-if="byCategory.length === 0" class="py-6 text-center text-sm text-gray-400">
                    No hay ventas registradas en esta fecha.
                </div>
                <div v-else class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-gray-100 text-left text-xs uppercase text-gray-400 dark:border-gray-800">
                                <th class="pb-2 pr-4 font-medium">Departamento</th>
                                <th class="pb-2 pr-4 text-right font-medium">Unidades</th>
                            ...
                            >
                                <td class="py-2 pr-4 font-medium text-gray-700 dark:text-gray-300">
                                    <span
                                        ...
                                        :style="{ backgroundColor: ['#FF2E7A', '#3B82F6', '#F59E0B', '#10B981', '#8B5CF6', '#EF4444', '#EC4899', '#14B8A6'][i % 8] }"
                                    />
                                    {{ item.category }}
                                </td>
                                <td class="py-2 pr-4 text-right text-gray-500">{{ item.qty }}</td>
                                <td class="py-2 text-right font-semibold text-gray-800 dark:text-white">{{ fmt(item.total) }}</td>
                            </tr>
                            <tr class="border-t-2 border-gray-200 font-bold dark:border-gray-700">
                                <td class="py-2 pr-4 text-gray-800 dark:text-white">Total</td>
                                <td class="py-2 pr-4 text-right text-gray-800 dark:text-white">
                                    {{ byCategory.reduce((a, c) => a + c.qty, 0) }}
                                </td>
                                <td class="py-2 text-right text-gray-800 dark:text-white">
                                    {{ fmt(byCategory.reduce((a, c) => a + c.total, 0)) }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <div class="rounded-2xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-surface-dark">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                    <div class="rounded-xl bg-gray-50 p-4 dark:bg-gray-800/50">
                        <p class="text-xs text-gray-400">Ventas Totales (Bruto)</p>
                        <p class="text-2xl font-bold text-gray-800 dark:text-white">{{ fmt(summary.grossSales) }}</p>
                    </div>
                    <div class="rounded-xl bg-primary-50 p-4 dark:bg-primary-900/20">
                        <p class="text-xs text-primary-600 dark:text-primary-400">Total General (Apertura + Ventas)</p>
                        <p class="text-2xl font-bold text-primary-700 dark:text-primary-300">{{ fmt(summary.totalGeneral) }}</p>
                    </div>
                    <div class="rounded-xl bg-emerald-50 p-4 dark:bg-emerald-900/20">
                        <p class="text-xs text-emerald-600 dark:text-emerald-400">Ganancia del Día (Neta)</p>
                        <p class="text-2xl font-bold" :class="profitColor">{{ fmt(summary.netProfit) }}</p>
                    </div>
                    <div class="rounded-xl bg-amber-50 p-4 dark:bg-amber-900/20">
                        <div class="mb-1 flex items-center gap-1.5">
                            <Percent class="h-4 w-4 text-amber-500" />
                            <p class="text-xs text-amber-600 dark:text-amber-400">Margen de Utilidad</p>
                        </div>
