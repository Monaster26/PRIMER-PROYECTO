...
const fmt = (v: number | null | undefined) =>
    v != null ? '$' + v.toLocaleString('es-CO') : null;
...
watch(showDetailModal, (val) => {
    if (val && detailItem.value?.metadata) {
        detailDesgloseAnterior.value =
            detailItem.value.metadata.old_desglose ?? null;
        detailDesgloseNuevo.value =
            detailItem.value.metadata.new_desglose ?? null;
...
function saveNote() {
    ...
    router.put(
        route('admin.observaciones.update', noteItem.value.id),
        {
    ...
    <AdminLayout>
        <div class="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-content-muted">
                        Registro de eliminación de ítems, limpieza de carrito y
                        descuadres de apertura en Caja Rápida
                    </p>
                </div>
            </div>
            ...
            >
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        ...
                        >
                            ...
                            >
                                <td
                                    class="px-4 py-3 text-sm font-medium text-content-primary dark:text-white"
                                >
                                    {{ o.user?.name ?? '—' }}
                                </td>
                                <td class="px-4 py-3">
                                    <span
                                        ...
                                        :class="
                                            accionColor[o.tipo_accion] ??
                                            'bg-gray-100 text-gray-700'
                                        "
                                    >
                                        <component
                                            :is="accionIcon[o.tipo_accion]"
                                            class="h-3.5 w-3.5"
                                        />
                                        {{
                                            accionLabel[o.tipo_accion] ??
                                            o.tipo_accion
                                        }}
                                    </span>
                                </td>
                                <td
                                    class="max-w-xs truncate px-4 py-3 text-sm text-content-secondary"
                                >
                                    {{ o.producto_afectado ?? o.detalle }}
                                </td>
                                <td
                                    class="whitespace-nowrap px-4 py-3 text-right text-sm font-bold text-content-primary dark:text-white"
                                >
                                    {{ fmt(o.monto_diferencia) ?? '—' }}
                                </td>
                                <td
                                    class="whitespace-nowrap px-4 py-3 text-sm text-content-muted"
                                >
                                    {{ formatDateTime(o.created_at) }}
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 text-sm">
                                    <span
                                        v-if="o.estado === 'revisado'"
                ...
                <!-- Pagination -->
                <div
                    v-if="observaciones.last_page > 1"
                    class="flex items-center justify-between border-t border-gray-100 px-4 py-3 dark:border-gray-800"
                >
                    <span class="text-xs text-content-muted">
                        Página {{ observaciones.current_page }} de
                        {{ observaciones.last_page }}
                    </span>
                    <div class="flex gap-1">
                        <Link
                            v-if="observaciones.prev_page_url"
                            :href="observaciones.prev_page_url"
                        ...
                        <Link
                            v-if="observaciones.next_page_url"
                            :href="observaciones.next_page_url"
                            class="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-bold text-content-secondary transition-colors hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800"
                        >
                            Siguiente
                        </Link>
                    ...
                    <div class="max-h-[70vh] space-y-4 overflow-y-auto p-5">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            <span
                                class="inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-xs font-bold"
                                :class="
                                    accionColor[detailItem?.tipo_accion] ??
                                    'bg-gray-100 text-gray-700'
                                "
                            >
                                <component
                                    :is="accionIcon[detailItem?.tipo_accion]"
                                    class="h-3.5 w-3.5"
                                />
                                {{
                                    accionLabel[detailItem?.tipo_accion] ??
                                    detailItem?.tipo_accion
                                }}
                            </span>
                            <span class="text-xs text-content-muted"
                        ...
                        >
                            ...
                            >
                                <table class="w-full text-left text-xs">
                                    <thead
                                        class="bg-gray-50 uppercase tracking-wider text-content-muted dark:bg-gray-900/50"
                                    >
                                        <tr>
                                    ...
                                    >
                                        ...
                                        >
                                            <td
                                                class="px-3 py-1.5 font-semibold text-content-primary dark:text-white"
                                            >
                                                {{ d.label }}
                                            </td>
                                            <td
                                                class="px-3 py-1.5 text-center text-content-muted"
                                            >
                                                {{
                                                    qtyFromDesglose(
                                                        detailDesgloseAnterior,
                                            ...
                                            <td
                                                class="px-3 py-1.5 text-right font-bold"
                                                :class="
                                                    qtyFromDesglose(
                                                        detailDesgloseAnterior,
                                                        d.key,
                                    ...
                                    <tfoot
                                        class="bg-gray-50 text-xs font-bold dark:bg-gray-900/50"
                                    >
                                        <tr>
                                            <td
                                                class="px-3 py-2 text-content-primary dark:text-white"
                                            >
                                                TOTAL
                                            </td>
                                            <td
                                                class="px-3 py-2 text-center text-content-muted"
                                            >
                                                {{ fmt(detailOldTotal) }}
                                            </td>
                                            <td
                        ...
                        <template v-else>
                            ...
                            >
                                <p
                                    class="whitespace-pre-wrap text-sm text-content-secondary"
                                >
                                    {{ detailItem?.detalle }}
                                </p>
                            </div>
                            <p
                                v-if="detailItem?.producto_afectado"
                                class="text-xs text-content-muted"
                            >
                                Producto afectado:
                                <strong>{{
                                    detailItem.producto_afectado
                                }}</strong>
                            </p>
                        </template>
                        ...
                        <div
                            v-if="detailItem?.nota_admin"
                            class="rounded-xl bg-primary-50 p-3 dark:bg-primary-900/10"
                        >
                            <p
                            ...
                            <p
                                class="mt-1 text-sm text-content-primary dark:text-white"
                            >
                                {{ detailItem.nota_admin }}
                            </p>
                        </div>
                    </div>
                </div>
        ...
        >
            ...
            >
                <div
                    class="w-full max-w-lg rounded-2xl bg-white shadow-xl dark:bg-surface-dark"
                >
                    <div
                        class="flex items-center justify-between border-b border-gray-100 px-5 py-3 dark:border-gray-800"
                    >
                        ...
                        >
                            Agregar Nota —
                            {{ accionLabel[noteItem?.tipo_accion] ?? '' }}
                        </h3>
                        <button
                            @click="closeNote"
                            class="rounded-lg p-1 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800"
                    ...
                    <div class="space-y-4 p-5">
                        <p class="text-xs text-content-muted">
                            Revisando:
                            <strong>{{ noteItem?.user?.name }}</strong> —
                            {{ formatDateTime(noteItem?.created_at) }}
                        </p>
                        <p
                            class="text-xs leading-relaxed text-content-secondary"
                        >
                            {{ noteItem?.detalle }}
                        </p>
                        <textarea
                            ...
                            rows="4"
                            placeholder="Escribe aquí el resultado de tu revisión..."
                        ></textarea>
