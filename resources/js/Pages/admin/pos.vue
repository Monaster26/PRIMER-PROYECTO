...
const {
    ...
    csrfToken,
    props.hasOpenSession,
    computed(() => props.ultimaSesion),
);
...
const {
    ...
    decrementQty,
    removeItem,
} = useCart(cartRef, props.products);
...
const cashAmount = computed(
    () =>
        activeTab.value.payments.find((p) => p.method === 'cash')?.amount ??
        null,
);
const cardAmount = computed(
    () =>
        activeTab.value.payments.find((p) => p.method === 'card')?.amount ??
        null,
);
const transferAmount = computed(
    () =>
        activeTab.value.payments.find((p) => p.method === 'transfer')?.amount ??
        null,
);
const totalUnits = computed(() =>
    activeTab.value.cart.reduce((s, i) => s + i.quantity, 0),
);

function setPaymentAmount(method: string, v: number | null) {
    const p = activeTab.value.payments.find((p) => p.method === method);
    if (p) p.amount = v;
}
...
watch(total, () => {
    ...
    const transfer = activeTab.value.payments.find(
        (p) => p.method === 'transfer',
    );
...
function confirmClearCart() {
    if (activeTab.value.cart.length) {
        const lineas = activeTab.value.cart.map(
            (i) =>
...
function handleMenuAction(label: string) {
    if (label === 'Entrada de Dinero') {
        cashMovementType.value = 'ingreso';
        showCashMovementModal.value = true;
    } else if (label === 'Salida de Dinero') {
        cashMovementType.value = 'retiro';
        showCashMovementModal.value = true;
    } else if (label === 'Cerrar Caja') {
        if (activeTab.value.cart.length > 0) showCartWarning.value = true;
        else showCloseSessionModal.value = true;
    }
}
...
function handleKeydown(e: KeyboardEvent) {
    if (e.key === 'F9') {
        e.preventDefault();
        if (canCheckout.value) handleFinalizeSale();
    }
}
</script>
    ...
    <AdminLayout>
        ...
        <div v-if="sessionOpened">
            ...
            <div class="flex h-[calc(100vh-10rem)] gap-4">
                ...
                <PosPaymentPanel
                    ...
                    @update:cash-amount="
                        (v: number | null) => setPaymentAmount('cash', v)
                    "
                    @update:card-amount="
                        (v: number | null) => setPaymentAmount('card', v)
                    "
                    @update:transfer-amount="
                        (v: number | null) => setPaymentAmount('transfer', v)
                    "
        ...
        >
            <div
                ...
                @click.self="showCartWarning = false"
            >
                <div
